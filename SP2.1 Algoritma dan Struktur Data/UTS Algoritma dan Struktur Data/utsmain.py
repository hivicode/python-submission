...
class transaksi:
    def __init__(self):
        ...
        self.totalharga = 0
        self.stack_hapus = []
        self.queue_pelanggan = deque() 

    def tambah_pelanggan(self, nama):
        self.queue_pelanggan.append(nama)
        print(f"\nPelanggan '{nama}' ditambahkan ke antrian.\n")

    def proses_pelanggan(self):
        if self.queue_pelanggan:
            pelanggan = self.queue_pelanggan.popleft()
            print(f"\nPelanggan '{pelanggan}' sedang diproses.\n")
        else:
    ...
    def hapus_barang(self, barang):
        try:
            # Simpan barang yang dihapus ke stack
            self.stack_hapus.append((barang, self.keranjang[barang]))
            del self.keranjang[barang]
            print(f"\nBarang '{barang}' berhasil dihapus dan disimpan ke riwayat.\n")
        except KeyError:
            print("\nBarang tidak ada di keranjang\n")

    def kembalikan_barang(self):
        if self.stack_hapus:
            barang, data = self.stack_hapus.pop()
            self.keranjang[barang] = data
            print(f"\nBarang '{barang}' berhasil dikembalikan ke keranjang.\n")
        else:
    ...
    def header(self):
        ...
        print("{:<9}: {:<9}".format("Kasir", "Bintang"))
